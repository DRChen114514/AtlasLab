...
const onboardingSteps = [
  { title: "检查运行环境", subtitle: "确认 AtlasLab 所需工具已经就绪" },
...
function renderSettings() {
  ...
  else if (settingsTab === "mineru") renderSettingsMineru();
...
// ===== MinerU Settings =====
function renderSettingsMineru() {
  settingsContent.innerHTML = `
    <div class="field">
      <span>MinerU API URL</span>
      <input id="setMineruUrl" value="${escapeHtml(settingsConfig.mineruUrl || 'http://localhost:8080')}" />
      <span class="field-hint">opendatalab/MinerU 服务地址，用于高精度 PDF 解析</span>
    </div>
    <div class="field">
      <span>MinerU API Key</span>
      <div style="display:flex;gap:8px;">
        <input id="setMineruKey" type="password" value="${escapeHtml(settingsConfig.mineruKey || '')}" style="flex:1;" />
        <button class="ghost-button" type="button" id="setMineruToggleKey">👁 显示</button>
        <button class="primary-button" type="button" id="setMineruTestBtn">🔍 测试</button>
      </div>
      <div id="setMineruTestResult"></div>
    </div>
    <div class="toggle-row">
      <div>
        <strong>启用 MinerU 解析</strong>
        <p class="field-hint">关闭后自动回退到本地 PyMuPDF/pdfplumber 解析</p>
      </div>
      <div class="toggle-switch ${settingsConfig.mineruEnabled ? 'on' : ''}" id="setMineruToggle"></div>
    </div>
    <div class="field">
      <span>MinerU 超时（秒）</span>
      <input id="setMineruTimeout" type="number" value="${settingsConfig.mineruTimeout || 120}" />
    </div>
  `;
  document.querySelector("#setMineruToggleKey")?.addEventListener("click", (e) => {
    const input = document.querySelector("#setMineruKey");
    if (input.type === "password") { input.type = "text"; e.target.textContent = "👁 隐藏"; }
    else { input.type = "password"; e.target.textContent = "👁 显示"; }
  });
  document.querySelector("#setMineruTestBtn")?.addEventListener("click", async () => {
    const url = document.querySelector("#setMineruUrl").value;
    const key = document.querySelector("#setMineruKey").value;
    const result = document.querySelector("#setMineruTestResult");
    result.innerHTML = '<p class="field-hint">测试中...</p>';
    try {
      const r = await fetch(`${url}/health`, { headers: key ? { Authorization: `Bearer ${key}` } : {} });
      result.innerHTML = r.ok
        ? '<div class="test-result ok">✓ MinerU 连接成功</div>'
        : `<div class="test-result fail">✗ HTTP ${r.status}</div>`;
    } catch(e) {
      result.innerHTML = `<div class="test-result fail">✗ ${escapeHtml(e.message)}</div>`;
    }
  });
}
