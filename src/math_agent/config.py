...
# 返回 5xx、连接失败或超时时，AtlasLab 还需要像 Codex/Claude CLI 一样切换模型，
...
# ---- Document Intelligence (MinerU) configuration ----
# MinerU API: high-accuracy PDF parsing service (opendatalab/MinerU)
# Auto-fallback to local PyMuPDF/pdfplumber when unconfigured
MINERU_ENABLED = os.getenv("MINERU_ENABLED", "0") == "1"
MINERU_API_URL = os.getenv("MINERU_API_URL", "http://localhost:8080")
MINERU_API_KEY = os.getenv("MINERU_API_KEY", "")
MINERU_TIMEOUT = int(os.getenv("MINERU_TIMEOUT", "120"))

# ---- Knowledge Base configuration ----
# Multi-knowledge-base storage paths (model library, papers, textbooks, algorithm templates, etc.)
KNOWLEDGE_BASE_DIR = os.getenv(
    "MATH_AGENT_KNOWLEDGE_BASE_DIR", str(PROJECT_ROOT / "runs" / "knowledge")
)
# Hybrid retrieval weights (vector search : BM25)
HYBRID_RETRIEVAL_VECTOR_WEIGHT = float(
    os.getenv("MATH_AGENT_HYBRID_VECTOR_WEIGHT", "0.7")
)
HYBRID_RETRIEVAL_BM25_WEIGHT = float(
    os.getenv("MATH_AGENT_HYBRID_BM25_WEIGHT", "0.3")
)

# ---- Experiment Center configuration ----
# Experiment manager workspace path
EXPERIMENT_WORKSPACE = os.getenv(
    "MATH_AGENT_EXPERIMENT_WORKSPACE", str(PROJECT_ROOT / "runs" / "experiments")
)
# Auto ablation: run ablation study after every model change
AUTO_ABLATION = os.getenv("MATH_AGENT_AUTO_ABLATION", "0") == "1"
# Fixed random seed policy for reproducibility
EXPERIMENT_DEFAULT_SEED = int(os.getenv("MATH_AGENT_EXPERIMENT_SEED", "42"))

# ---- Reviewer configuration ----
# Minimum scores for automated reviewer approval (0-10)
MIN_MATH_REVIEWER_SCORE = _quality_score("MATH_AGENT_MIN_MATH_REVIEWER_SCORE", "7")
MIN_LOGIC_REVIEWER_SCORE = _quality_score("MATH_AGENT_MIN_LOGIC_REVIEWER_SCORE", "7")
MIN_WRITING_REVIEWER_SCORE = _quality_score("MATH_AGENT_MIN_WRITING_REVIEWER_SCORE", "8")
MIN_COMPETITION_REVIEWER_SCORE = _quality_score(
    "MATH_AGENT_MIN_COMPETITION_REVIEWER_SCORE", "8"
)
MIN_INNOVATION_REVIEWER_SCORE = _quality_score(
    "MATH_AGENT_MIN_INNOVATION_REVIEWER_SCORE", "6"
)
